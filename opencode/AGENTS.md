...
- Never hand-roll a workflow a skill already encodes. Plugins active in this
  environment: superpowers (skill workflow) and ponytail (laziness) — follow them.
- Skills auto-load from ~/.claude/skills, ~/.agents/skills, superpowers. If none
  applies, say so in one line and proceed — no stalling on skill hunting.
